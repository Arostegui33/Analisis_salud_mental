...
png("resultados/clustering/wss_clusters_pais_vasco.png", width = 800, height = 600)
plot(1:10, wss, type = "b", pch = 19,
     xlab = "Número de Clústeres (k)",
     ylab = "Suma de cuadrados intra-cluster (WSS)",
     main = "Método del Codo - k óptimo (País Vasco)")
...
# Perfil medio por clúster
write.csv(round(perfil_cluster,2), "resultados/clustering/perfil_medio_clusters_pais_vasco.csv", row.names = FALSE)

# Heatmap de perfiles
png("resultados/clustering/heatmap_clusters_pais_vasco.png", width = 800, height = 600)
pheatmap(perfil_cluster_t,
         cluster_rows = TRUE,
         cluster_cols = TRUE,
         main = "Perfil promedio por clúster - País Vasco",
...
png("resultados/clustering/kmeans_clusters_pais_vasco.png", width = 900, height = 700)
fviz_cluster(modelo_kmeans_pv, data = datos_pv_cluster,
             geom = "point", ellipse.type = "norm",
             palette = "Set2", ggtheme = theme_minimal(),
             main = "Clustering K-means - País Vasco")
...
# Visualización PCA
png("resultados/clustering/pca_clusters_pais_vasco.png", width = 900, height = 700)
ggplot(pca_data, aes(x = PC1, y = PC2, color = cluster)) +
  geom_point(alpha = 0.7, size = 2) +
  labs(title = "Clustering K-means en País Vasco (PCA)",
       x = "PC1", y = "PC2") +
...
# Distribución sociodemográfica por clúster
# Por género
png("resultados/clustering/distribucion_genero_clusters_pais_vasco.png", width = 900, height = 700)
ggplot(sexo_df, aes(x = Cluster, y = Frecuencia, fill = Genero)) +
  geom_bar(stat = "identity", position = "fill") +
  labs(title = "Distribución de género por clúster - País Vasco",
       y = "Porcentaje", x = "Clúster") +
...
# Por provincia
png("resultados/clustering/distribucion_provincia_clusters_pais_vasco.png", width = 900, height = 700)
ggplot(prov_df, aes(x = Cluster, y = Frecuencia, fill = Provincia)) +
  geom_bar(stat = "identity", position = "fill") +
  labs(title = "Distribución de provincia por clúster - País Vasco",
       y = "Porcentaje", x = "Clúster") +
...
# Por grupo de edad
png("resultados/clustering/distribucion_edad_clusters_pais_vasco.png", width = 900, height = 700)
ggplot(edad_df, aes(x = Cluster, y = Frecuencia, fill = GrupoEdad)) +
  geom_bar(stat = "identity", position = "fill") +
  labs(title = "Distribución por grupo de edad por clúster - País Vasco",
       y = "Porcentaje", x = "Clúster") +
